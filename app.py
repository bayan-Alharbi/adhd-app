...
<style>
    ...
    /* ── App Background ── */
    .stApp {
        background: linear-gradient(135deg, #0D1B2E 0%, #122444 50%, #0D1B2E 100%) !important;
    ...
    [data-testid="stSidebar"] .stRadio label:hover {
        background: rgba(75,163,212,0.12) !important;
    }
    ...
    [data-testid="stMetric"] {
        background: linear-gradient(135deg, var(--bg-card), #1A3560) !important;
    ...
    .stButton > button[data-testid="baseButton-primary"] {
        background: linear-gradient(135deg, var(--orange-red) 0%, var(--orange) 100%) !important;
        color: #fff !important;
        border: none !important;
        ...
        box-shadow: 0 4px 20px rgba(224,90,28,0.35) !important;
        transition: all 0.2s !important;
    }
    .stButton > button[kind="primary"]:hover {
        transform: translateY(-2px) !important;
        box-shadow: 0 6px 28px rgba(224,90,28,0.5) !important;
    }
    ...
    .stDownloadButton > button {
        background: linear-gradient(135deg, var(--navy-light), var(--sky)) !important;
        color: #fff !important;
        border: none !important;
        border-radius: 10px !important;
        font-weight: 700 !important;
        box-shadow: 0 4px 18px rgba(75,163,212,0.3) !important;
    }
    ...
    .stNumberInput > div > div > input:focus {
        ...
        box-shadow: 0 0 0 3px rgba(75,163,212,0.15) !important;
    }
    ...
    [data-testid="stFileUploader"] {
        background: var(--bg-card) !important;
        border: 2px dashed rgba(75,163,212,0.35) !important;
        border-radius: 14px !important;
    ...
    .stError {
        background: rgba(224,90,28,0.12) !important;
    ...
    .cover-title {
        ...
        text-align: center;
        margin-top: 0.5rem;
        background: linear-gradient(135deg, var(--orange) 0%, var(--orange-red) 40%, var(--sky) 100%);
        -webkit-background-clip: text;
        -webkit-text-fill-color: transparent;
    ...
    /* ─── FACT CARDS ─── */
    .fact-card {
        background: linear-gradient(135deg, var(--bg-card) 0%, #1A3560 100%);
    ...
    /* ─── STEP CARDS ─── */
    .step-card {
        background: linear-gradient(160deg, var(--bg-card) 0%, #142040 100%);
    ...
    .result-adhd {
        background: linear-gradient(135deg, rgba(224,90,28,0.15) 0%, rgba(224,90,28,0.05) 100%);
        border: 2px solid var(--orange-red);
    }
    .result-ctrl {
        background: linear-gradient(135deg, rgba(75,163,212,0.15) 0%, rgba(75,163,212,0.05) 100%);
    ...
    .section-tag {
        ...
        background: linear-gradient(90deg, var(--orange-red), var(--orange));
        color: #fff;
        font-size: 0.72rem;
        font-weight: 700;
...
# ── Gauge helper — branded colors ────────────────────────

def make_gauge(value, is_adhd):
    bar_color   = "#E05A1C" if is_adhd else "#4BA3D4"
    step_color1 = "rgba(75,163,212,0.12)"
    step_color2 = "rgba(224,90,28,0.12)"
    fig = go.Figure(go.Indicator(
        mode  = "gauge+number",
        value = value * 100,
        number= {"suffix": "%", "font": {"size": 42, "color": "#EDF2FF", "family": "Plus Jakarta Sans"}},
        title = {"text": "ADHD Likelihood", "font": {"size": 16, "color": "#8BA4C8", "family": "Plus Jakarta Sans"}},
        gauge = {
            "axis"     : {"range": [0, 100], "tickcolor": "#4BA3D4", "tickfont": {"color": "#8BA4C8"}},
            "bar"      : {"color": bar_color, "thickness": 0.28},
            "bgcolor"  : "rgba(0,0,0,0)",
            "borderwidth": 0,
            "steps"    : [
                {"range": [0,  50], "color": step_color1},
                {"range": [50, 100], "color": step_color2},
            ],
            "threshold": {"line": {"color": "#F5A623", "width": 3}, "value": 50}
        }
    ))
    fig.update_layout(
        height=320,
...
def generate_pdf_report(patient_name, patient_age, patient_gender,
    ...
    ORANGE_RED = colors.HexColor("#E05A1C")
    SKY        = colors.HexColor("#4BA3D4")
    NAVY       = colors.HexColor("#1A3A6B")
    BG_NAVY    = colors.HexColor("#EBF4FF")
    ...
    sub_style = ParagraphStyle("sub_s", parent=styles["Normal"],
        fontSize=12, textColor=SKY, alignment=TA_CENTER,
    ...
    diag_style = ParagraphStyle("diag", parent=styles["Normal"],
        fontSize=20, textColor=RED if meta_prob >= 0.5 else colors.HexColor("#1A7A4A"),
    ...
    if uploaded:
        ...
        try:
            ...
            if wins is not None and len(wins) > 0:
                ...
                st.plotly_chart(make_gauge(mean_prob, is_adhd), use_container_width=True)
...
# ══════════════════════════════════════════════════════════
    ...
    if p_eeg is not None or p_hyp is not None:
        ...
        st.plotly_chart(make_gauge(meta_prob, is_adhd), use_container_width=True)
        ...
        if is_adhd:
            ...
            <div class="result-box result-adhd">
                ...
                <div class="result-prob" style="margin-top:0.6rem;font-size:0.92rem;">
